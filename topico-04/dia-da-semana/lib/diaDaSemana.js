...
var dataDesejada = new _dataFormatada.DataFormatada(process.argv[2] ? process.argv[2] : "");
var anoBissexto = process.argv[3];
var dataReferencia = new _dataFormatada.DataFormatada(process.argv[4] ? process.argv[4] : "");
var diaDaSemanaDataReferencia = process.argv[5];
calcularData(dataDesejada);

function calcularData(dataDesejada) {
  validarArgumentos();
  var diaDaSemana = parseInt(diaDaSemanaDataReferencia);
...
function calcularDiaDaSemanaDataDesejadaMaiorQueDataReferencia(diaDaSemana) {
  while (dataDesejada.getDataInteger() != dataReferencia.getDataInteger()) {
    ...
    if (isAnoBissexto(dataReferencia.ano)) {
      if (dataReferencia.dia > _meses.MESES_BISSEXTO[dataReferencia.mes - 1]) {
        dataReferencia.dia = 1;
        dataReferencia.mes++;
      }
    } else if (dataReferencia.dia > _meses.MESES_NAO_BISSEXTO[dataReferencia.mes - 1]) {
      console.log("opa2");
      dataReferencia.dia = 1;
      dataReferencia.mes++;
    }
...
function calcularDiaDaSemanaDataDesejadaMenorQueDataReferencia(diaDaSemana) {
  while (dataDesejada.getDataInteger() != dataReferencia.getDataInteger()) {
    ...
    if (dataReferencia.dia === 0) {
      ...
      if (isAnoBissexto(dataReferencia.ano)) {
        dataReferencia.dia = _meses.MESES_BISSEXTO[dataReferencia.mes - 1];
      } else {
        dataReferencia.dia = _meses.MESES_NAO_BISSEXTO[dataReferencia.mes - 1];
      }
    }
    if (diaDaSemana < 0) {
      diaDaSemana = 6;
