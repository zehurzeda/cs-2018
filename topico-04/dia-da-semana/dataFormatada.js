export class DataFormatada {
    ...
    constructor(dataSemFormatacao) {
        ...
        this.getDataInteger = function () {
            let dataInteger = parseInt(this.ano.toString() +
                (pad.substring(0, pad.length - this.mes.toString().length) + this.mes.toString()) +
                (pad.substring(0, pad.length - this.dia.toString().length) + this.dia.toString()))
